...
twitch_miner = TwitchChannelPointsMiner(
    ...
    logger_settings=LoggerSettings(
        ...
        discord=Discord(
            webhook_api="https://discord.com/api/webhooks/1220102175066161192/V2vM_h0c3f_HEg4lQDz_YTfrurqyNQ3kJC4NfB5AuMCQQ-wy0_yL6UcieEzgeduV_Tsf",  # Discord Webhook URL
            events=[Events.STREAMER_ONLINE, Events.STREAMER_OFFLINE,
                    Events.BET_LOSE, Events.CHAT_MENTION],                                  # Only these events will be sent to the chat
        ),
        matrix=Matrix(
...
twitch_miner.mine(
    [
       Streamer("leothecrackfn", settings=StreamerSettings(  , follow_raid=True ,   , watch_streak=True, bet=BetSettings(strategy=Strategy.SMART      , percentage=5 , stealth_mode=True,  percentage_gap=20 , max_points=234   , filter_condition=FilterCondition(by=OutcomeKeys.TOTAL_USERS,      where=Condition.LTE, value=800 ) ) )),
       Streamer("jolavanille", settings=StreamerSettings( , follow_raid=True  ,       watch_streak=True, bet=BetSettings(strategy=Strategy.PERCENTAGE , percentage=5 , stealth_mode=False, percentage_gap=20 , max_points=1234  , filter_condition=FilterCondition(by=OutcomeKeys.TOTAL_POINTS,     where=Condition.GTE, value=250 ) ) )),
       Streamer("aminematue", settings=StreamerSettings , follow_raid=True  , ,       watch_streak=True,bet=BetSettings(strategy=Strategy.PERCENTAGE , percentage=5 , stealth_mode=False, percentage_gap=20 , max_points=1234  , filter_condition=FilterCondition(by=OutcomeKeys.TOTAL_POINTS,     where=Condition.GTE, value=250 ) ) )),
